...
namespace RawScaler {
	...
	//通过绝对值比较决定纵向还是和横向
	//高通滤波在图像处理中通常用于：提高图像的锐利度强调边缘和细节
	float RawProcessor::AdamsInterpolation(unsigned char* in, int x, int y, int width,float high_pass_weight = 1.0)//high_pass_weight:控制高通滤波的强度
    ...
    //  横向（direction = 1）和纵向（direction = 0）计算周围像素的值：
	//直接指定纵向横向
	...
	/*
	unsigned char* in：指向图像数据的指针，通常包含RGB值的图像。
	int x 和 int y：当前处理的像素的坐标。
	int width：图像的宽度，方便计算像素在一维数组中的索引。
	float max_cut：用于设定插值计算中的限制值，防止计算的值超出预定范围（默认为64）。
	int kernel_size：用于计算标准差和中值的窗口大小（默认为5），虽然在代码中实际使用3。
	*/
	float RawProcessor::Interpolation_G_Only(unsigned char* in, int x, int y, int width, float max_cut = 64,int kernel_size = 5)
	{
		float g_arr[8];//用于存储计算得到的绿色通道插值结果。
		float r, g, b;//用于存储红色、绿色和蓝色的值，虽然在该函数中主要聚焦于绿色。
		float mean[3][2];//未在函数中使用，可以忽略
	...
	{
		float w_ratio = iwidth / (float)owidth;// 缩放比例
		float h_ratio = iheight / (float)oheight;

		int dwidth = owidth;// 输出图像的宽度
		int dheight = oheight;
		float ratio = w_ratio * h_ratio;// 计算宽高比
		int rb_width = owidth;// 用于减少颜色通道的输出宽度
		int rb_height = oheight;// 用于减少颜色通道的输出高度
		if (w_ratio < 2 || h_ratio < 2) {
			//rb_width /= 2;
			//rb_height /= 2;
		}
		// 分别为G通道、GR通道和GB通道分配内存
