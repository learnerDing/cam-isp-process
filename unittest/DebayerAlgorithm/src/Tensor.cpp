    ...
    shape_[2] == (height_*width_ + 2*(height_/2)*(width_/2))) 
{
    ...
    {   //常规数据排列方式bgr各占4个字节
        ...
        #ifdef DBG_Tensor 
        printf("Tensor chw->hwc and scale\n");
        #endif
        // 转换为uint8（反归一化）
        cv::Mat output_mat;
        hwc_mat.convertTo(output_mat, CV_8UC(channels), scale);
        #ifdef DBG_Tensor 
        printf("Tensor convert to uint8 mat\n");
        #endif
        return output_mat;
    }
}
...
//打印函数重载，打印一行中某些元素而不是全行打印
void Tensor::print(int channel, int row, int start_col, int end_col, const std::string& name) const {
    print_shape(name);
    ...
    assert(channel >= 0 && channel < s[0] && "Channel index out of range");
    assert(row >= 0 && row < s[1] && "Row index out of range");
    assert(start_col >= 0 && start_col < s[2] && "Start column index out of range");
    assert(end_col >= 0 && end_col <= s[2] && "End column index out of range"); // end_col can be s[2]
    assert(start_col <= end_col && "Start column must be <= end column");

    int elements_to_print = end_col - start_col;

    // 数据指针定位
    const size_t offset = channel * s[1] * s[2] + row * s[2] + start_col;

    // 统一格式化输出
    std::cout << "Printing CHW tensor data [C=" << channel
              << ", H=" << row << ", W=" << start_col << "~" << (end_col - 1) << "]:\n";
    ...
    std::cout << "\n..." << (s[2] > end_col ? " (truncated row)" : "") // Indicate row truncation if end_col < s[2]
              << " Total elements in this row: " << s[2] << "\n\n";
}
