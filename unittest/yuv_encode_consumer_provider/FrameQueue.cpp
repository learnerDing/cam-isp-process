...
// 停止流 如果队列中还有未处理的 AVFrame，需要释放它们：
template <typename T>
void FrameQueue<T>::stop() {
    {
        ...
        while (!m_frameQueue.empty()) {
            T* frame = m_frameQueue.front();
            m_frameQueue.pop();
            if (getFrameType() == FRAME_TYPE_AVFRAME) {
                av_frame_free(&frame); // 如果队列里面存在的帧是AVFrame,释放 AVFrame
            } else {
                delete frame; // 释放其他类型的帧
            }
        }
    }
